function feature = get_sop_feature(depth_img,center_x, center_y, ...
  ...
  for bx = -num_bins(1)/2:num_bins(1)/2-1      
      ...
      for by= -num_bins(2)/2:num_bins(2)/2-1 
          ...
          %for bz =-num_bins(3)/2:num_bins(3)/2-1 
              ...
              %sub_z  = bz + num_bins(3)/2+1;
              sub_z  = 1;
              feature(sub_x,sub_y,sub_z) = min(points_sum,saturation_size);
              %feature(sub_x,sub_y,sub_z) =1/(1+ exp(-points_sum/saturation_size));
          %end
      end
  end  
end